    ...
    return (
        <>
                ...
                <Table hover variant="primary">
                    ...
                    <tbody>
                        {props.statement.accounts.map(x => {
                            ...
                            return (<>
                                ...
                                <tr><td className="py-2">
                                    <Row>
                                        <Col xs="1">{(month > 9 ? '' : '0') + month}　{(day > 9 ? '' : '0') + day}</Col>
                                        <Col xs="2" className="text-center"><span className={`bg-${x.amount > 0 ? "success" : "danger"} px-2 py-1 rounded text-white`}>{x.category}</span></Col>
                                        <Col xs="auto">{x.uploadBy}</Col>
                                        <Col><b>{x.name}</b></Col>
                                        {x.amount > 0 ?
                                            <>
                                                <Col xs={2} className="text-right text-success"><b className="pr-2">{x.amount}</b></Col>
                                                <Col xs={2} />
                                            </> :
                                            <Col xs={{ span: 2, offset: 2 }} className="text-right text-danger"><b className="pr-2">{x.amount * -1}</b></Col>
                                        }
                                    </Row>
                                </td></tr>
                            </>)
                        ...
                        <tr><th className="py-2 text-center bg-info">
                            <Row>
                                <Col>合　　　計</Col>
                                <Col xs="2" className="text-right"><span className="bg-light px-2 py-1 rounded text-success">{income}</span></Col>
                                <Col xs="2" className="text-right"><span className="bg-light px-2 py-1 rounded text-danger">{cost}</span></Col>
                            </Row>
                        </th></tr>
                    </tbody>
                </Table>
            ...
            <Row className="justify-content-center">
                <Col xs="auto">
                    <div className={`border px-3 rounded bg-${(income - cost) < 0 ? 'danger' : 'success'} text-white`}>
