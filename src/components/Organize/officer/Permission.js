...
export class Permission extends Component {
  ...
  update = (event) => {
    ...
    fetch(
      `http://${process.env.REACT_APP_DOMAIN}:${process.env.REACT_APP_PORT}/api/officers/update/permission`,
      {
        method: 'POST',
        headers: new Headers({
              ...
              <Form.Group as={Row}>
                ...
                <Col lg={5} className="py-1">
                  <Form.Control
                    ...
                    size="lg"
                    onChange={(event) => {
                      this.state[mapping[perm]][0] = event.target.value
                    }}
                  >
                    {this.props.officers.map((x) => {
                      if (
                        ...
                        this.state[mapping[perm]][0] = x.position
                        return (
                ...
                <Col lg={5} className="py-1">
                  <Form.Control
                    ...
                    size="lg"
                    onChange={(event) => {
                      this.state[mapping[perm]][1] = event.target.value
                    }}
                  >
                    <option value="">---------</option>
                    {this.props.officers.map((x) => {
                      if (
                        x.position !== this.state[mapping[perm]][0] &&
                        x.permission === perm
                      ) {
                        this.state[mapping[perm]][1] = x.position
                        return (
                          <option value={x.position} selected>
                            {x.position} ({x.sID})
                          </option>
