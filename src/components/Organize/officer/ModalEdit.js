...
export class ModalEdit extends Component {
  ...
  edit = (event) => {
    ...
    fetch("http://localhost:4000/api/officers/update/officer", {
      ...
      body: JSON.stringify({sID: this.state.sID, position: this.props.officer.position})
    });
        ...
        <Form onSubmit={this.edit}>
          ...
          <Modal.Body>
            <Form.Group as={Row}>
              ...
              <Col sm={9}>
                <Form.Control plaintext readOnly defaultValue={this.props.officer.position} className="font-weight-bold" />
              </Col>
            </Form.Group>
            <Form.Group as={Row}>
              <Form.Label column sm={3}>學　　號：</Form.Label>
              <Col sm={9}>
                <Form.Control type="number" defaultValue={this.props.officer.sID} name="sID" onChange={this.changeHandler} />
              </Col>
              <Col sm={{ offset: 3 }}>
          ...
          <Modal.Footer>
            ...
            <Button variant="light" onClick={this.props.onHide}>取消</Button>
          </Modal.Footer>
        </Form>
      </Modal>
    );
